...
struct benchmarker {
  ...
  really_inline void run_iteration(bool stage1_only, bool hotbuffers=false) {
    ...
    collector.start();
    error_code error = document::parser::get_implementation()->stage1((const uint8_t *)json.data(), json.size(), parser, false);
    ...
    if (stage1_only) {
      ...
      collector.start();
      error = document::parser::get_implementation()->stage2((const uint8_t *)json.data(), json.size(), parser);
      if (error) {
